 ...
 * (color encode), 3 lit (diffuse * lambert). Height/normal overlays blend over the
 * diffuse by authoredMask * opacity — unauthored regions stay transparent, previewing
 * exactly the alpha masking the NX export carries. All textures loaded at nearest
 * canvas px (pixelated under zoom, intentional).
 */
...
struct CompositeUniforms {
  ...
  canvasW: f32,
  canvasH: f32,
  opacity: f32,       // overlay opacity for height/normal modes (1 = pure overlay)
  heightMin: f32,
  heightMax: f32,
  lightX: f32,
  lightY: f32,
...
fn fs(@builtin(position) fragPos: vec4f) -> @location(0) vec4f {
  ...
  if (cu.mode == 1u) {
    let f = textureLoad(fieldTex, px, 0);
    let g = clamp((f.r - cu.heightMin) / max(cu.heightMax - cu.heightMin, 1e-6), 0.0, 1.0);
    return vec4f(mix(diffuse.rgb, vec3f(g), f.g * cu.opacity), 1.0);
  }
