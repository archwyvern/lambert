...
function slabWithKeep(): ObjectInstance {
  const slab = createObjectInstance(ObjectTypeId.Plateau, v(32, 32)); // covers most of a 64x64 canvas
...
describe("mask trim (CPU)", () => {
  it("a keep mask zeroes height + NX mask outside the loop, keeps them inside", () => {
    const f = evaluateField(resolveObjects([slabWithKeep()]), 64, 64);
  ...
  it("a world mask lands at the same doc location at ss1 and ss2 (supersample scaling)", () => {
    const r1 = renderField(resolveObjects([slabWithKeep()]), 64, 64, { supersample: 1 });
    const r2 = renderField(resolveObjects([slabWithKeep()]), 64, 64, { supersample: 2 });
