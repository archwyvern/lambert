...
// Frustum = Pipe with a flat cap + taper (radius2); length 64 (half 32), radius 16 (-x), radius2 8 (+x)
const frustum = getObjectType(ObjectTypeId.Pipe);
const inst = createObjectInstance(ObjectTypeId.Pipe, v2(0, 0));
inst.params.cap = "flat";
inst.params.radius2 = 8;
