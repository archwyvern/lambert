...
test("round-trips screen<->canvas", () => {
  ...
  const s = canvasToScreen(v, { x: 10, y: 20 });
...
test("zoomAt clamps to [0.125, 16]", () => {
  ...
  expect(zoomAt(v, { x: 0, y: 0 }, 4).zoom).toBe(16);
  expect(zoomAt({ ...v, zoom: 0.125 }, { x: 0, y: 0 }, 0.5).zoom).toBe(0.125);
});
...
test("fitViewport centers the canvas with margin", () => {
  ...
  expect(canvasToScreen(v, { x: 50, y: 25 })).toEqual({ x: 500, y: 250 }); // canvas center -> view center
});
